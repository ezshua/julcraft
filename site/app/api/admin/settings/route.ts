...
export async function PUT(request: Request) {
  ...
  if (!Array.isArray(items) || items.length === 0) {
    return Response.json({ error: t(dict, "admin.errors.settingsBadArray" as DictionaryKey) }, { status: 400 });
  }

  for (const raw of items) {
    const parsed = itemSchema.safeParse(raw);
    if (!parsed.success) {
      return Response.json({ error: t(dict, "admin.errors.settingsBadItem" as DictionaryKey) }, { status: 400 });
    }
    const { key, value } = parsed.data;
    if (!KNOWN_KEYS.has(key)) {
      return Response.json({ error: `${t(dict, "admin.errors.settingsUnknownKey" as DictionaryKey).replace("{key}", key)}` }, { status: 400 });
    }
    if (JSON_KEYS.has(key)) {
      try {
        JSON.parse(value);
      } catch {
        return Response.json({ error: `${t(dict, "admin.errors.settingsBadJson" as DictionaryKey).replace("{key}", key)}` }, { status: 400 });
      }
    }
  }
